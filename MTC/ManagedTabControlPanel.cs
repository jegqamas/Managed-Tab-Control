...
namespace MTC
{
    ...
    public partial class ManagedTabControlPanel : Control
    {
        ...
        private void CalculateDrawViraibles()
        {
            ...
            tabHeight = charSize.Height + tabh;
            tabTextOffset = tabh / 2;
        }
        /// <summary>
        ...
        public int CalculateTabPagesWidth()
        {
            ...
            if (TabPages != null)
            {
                foreach (IMTCTabPage page in TabPages)
                {
        ...
        private MTCDrawPageObjects GetDrawObjects(IMTCTabPage page)
        {
        ...
        protected override void OnPaint(PaintEventArgs pe)
        {
            ...
            {
                pe.Graphics.DrawLine(new Pen(new SolidBrush(TabPageSplitColor)), new Point(0, this.Height - 4),
                ...
                if (cX >= HScrollOffset)
                {
                    ...
                    if (SelectedTabPageIndex == i)
                    {
                        ...
                        pe.Graphics.FillRectangle(new SolidBrush(TabPageSelectedColor),
                                new Rectangle(x - HScrollOffset + 1, 0, width, this.Height));
                    }
                    else
                    {
                    ...
                    if (drawArgs.Text.Length > 0)
                    {
                        ...
                        if (!DrawCloseBoxOnEachPage) textWidth = width - 3 - textc;
            ...
            // Draw virtical line on button when a page is selected
            if (SelectedTabPageIndex >= 0)
            {
                pe.Graphics.DrawLine(new Pen(new SolidBrush(TabPageSelectedColor), 3), new Point(0, this.Height - 2),
                   new Point(this.Width, this.Height - 2));
               // pe.Graphics.DrawLine(new Pen(new SolidBrush(TabPageSplitColor)), new Point(this.Width - 1, this.Height - 3),
               //   new Point(this.Width - 1, this.Height));
               // pe.Graphics.DrawLine(new Pen(new SolidBrush(TabPageSplitColor)), new Point(0, this.Height - 3),
                    ...
                    if (SelectedTabPageIndex == highlightedTabPageIndex)
                    {
                        if ((e.X - mouseDownPoint.X) > 3 || (e.X - mouseDownPoint.X) < 3)
                        {
                            ...
                            if (newIndex >= 0)
                            {
                                ...
                                {
                                    IMTCTabPage currentPage = TabPages[SelectedTabPageIndex];
